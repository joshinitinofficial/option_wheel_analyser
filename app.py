...
if raw_text.strip():
    ...
    # ============================
    r = st.columns(5)
    card(r[0], "Total Capital", f"₹{total_capital:,.0f}")
    card(r[1], "Final Profit", f"₹{final_profit:,.0f}")
    card(r[2], "Avg Monthly Profit", f"₹{avg_monthly_profit:,.0f}")
    card(r[3], "Avg Monthly Profit %", f"{avg_monthly_profit_pct:.2f}%")
    card(r[4], "Drawdown", drawdown_text)
    ...
    ax1.text(0.5, 0.96, "Equity Curve",
